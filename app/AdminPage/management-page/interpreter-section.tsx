...
import { Loader2, Save, Palette, Users, Clock } from "lucide-react";
...
export default function InterpreterManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="shadow-sm rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Current Interpreters</p>
                <p className="text-3xl font-bold">{currentInterpreters.length}</p>
              </div>
        ...
        <Card className="shadow-sm rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Historic Interpreters</p>
                <p className="text-3xl font-bold">{historicInterpreters.length}</p>
              </div>
        ...
        <Card className="shadow-sm rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Colors Assigned</p>
                <p className="text-3xl font-bold">{Object.keys(colors).filter(k => colors[k]).length}</p>
              </div>
      ...
      {/* Interpreter Colors Section */}
      <Card className="shadow-sm rounded-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Interpreter Colors</CardTitle>
            <Button 
              onClick={handleSave} 
              disabled={saving}
        ...
        <CardContent>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <h4 className="text-md font-medium text-gray-900">Current Interpreters</h4>
          ...
          {historicInterpreters.length > 0 && (
            <div className="space-y-4 mt-6">
              <div className="flex items-center gap-2">
                <h4 className="text-md font-medium text-gray-900">Historic Interpreters</h4>
...
function InterpreterCard({ 
  ...
  return (
    <Card className={`relative hover:shadow-md transition-shadow ${isHistoric ? 'opacity-75' : ''}`}>
      <CardContent className="p-4">
        <div className="flex justify-between items-center">
          {/* Left side - Name and ID */}
          <div>
            <p className="font-semibold text-gray-900">{interpreter.name}</p>
            <Badge variant="outline" className="w-fit mt-1">
              ID: {interpreter.id}
            </Badge>
            {isHistoric && (
              <Badge variant="secondary" className="w-fit mt-1 ml-2 bg-gray-100 text-gray-600">
                Historic
              </Badge>
            )}
          </div>
          ...
          <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
            <PopoverTrigger asChild>
              <button className="p-1 rounded-lg hover:bg-gray-50 transition-colors">
                <div
                  className="w-8 h-8 rounded-full border border-gray-300 cursor-pointer hover:scale-105 transition-transform shadow-sm"
