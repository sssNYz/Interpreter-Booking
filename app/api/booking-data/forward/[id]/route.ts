...
async function isEnvInterpreterCapacityFull(envId: number, timeStart: Date, timeEnd: Date): Promise<boolean> {
  // Total interpreters in environment
  const total = await prisma.environmentInterpreter.count({ where: { environmentId: envId } });
...
async function isWithinUrgentThreshold(envId: number | null, meetingType: string, timeStart: Date): Promise<boolean> {
  try {
    const priority = envId != null
      ? await getEnvMeetingTypePriority(envId, meetingType)
      : null;
    const urgentDays = priority?.urgentThresholdDays ?? 1;
    const now = new Date();
    const diffDays = Math.floor((timeStart.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isFinite(bookingId)) return NextResponse.json({ ok: false, error: "Invalid booking id" }, { status: 400 });
  ...
  let targets = Array.isArray(body?.targets) ? body!.targets.filter((n) => Number.isFinite(n)) : [];
  ...
  if (booking.bookingStatus !== 'waiting') return NextResponse.json({ ok: false, error: "Only waiting bookings can be forwarded" }, { status: 409 });

  // Determine user's environment via center
  const myCenter = centerPart(me.deptPath ?? null);
  let myEnvId: number | null = null;
  if (myCenter) {
    const envCenter = await prisma.environmentCenter.findUnique({ where: { center: myCenter }, select: { environmentId: true } });
  ...
  const capacityFull = myEnvId != null ? await isEnvInterpreterCapacityFull(myEnvId, timeStart, timeEnd) : false;

  // Urgent threshold check (user's environment)
  const urgent = await isWithinUrgentThreshold(myEnvId, booking.meetingType as string, timeStart);

  if (!capacityFull && !urgent) {
    return NextResponse.json({ ok: false, error: "Not eligible to forward", reasons: { capacityFull, urgent } }, { status: 400 });
  }

  // If no targets provided, compute defaults: all active environments with admins, excluding user's environment
  if (targets.length === 0) {
    const envs = await prisma.environment.findMany({
      where: { isActive: true, NOT: myEnvId != null ? { id: myEnvId } : undefined },
      select: { id: true, admins: { select: { id: true } } }
    });
    targets = envs.filter(e => (e.admins?.length || 0) > 0).map(e => e.id);
  }
  if (targets.length === 0) return NextResponse.json({ ok: false, error: "No eligible environments to forward" }, { status: 400 });
  ...
  try {
    const saved = await prisma.$transaction(async (tx) => {
      ...
      await tx.bookingForwardTarget.createMany({
        data: targets.map((envId) => ({ bookingId, environmentId: Number(envId) })),
        skipDuplicates: true,
      });

      const fa = Array.isArray(booking.forwardActions) ? (booking.forwardActions as unknown[]) : [];
      fa.push({ actor: 'user', empCode: me.empCode, action: 'FORWARD', at: new Date().toISOString(), note: note || null });
      await tx.bookingPlan.update({ where: { bookingId }, data: { isForwarded: true, forwardedByEmpCode: me.empCode, forwardActions: fa as Prisma.InputJsonValue } });

      const targetsRows = await tx.bookingForwardTarget.findMany({ where: { bookingId }, select: { environmentId: true } });
      return targetsRows.map((r) => r.environmentId);
    });

    return NextResponse.json({ ok: true, data: { bookingId, capacityFull, urgent, targets: saved } });
  } catch (e) {
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const booking = await prisma.bookingPlan.findUnique({
    where: { bookingId },
    select: { bookingId: true, bookingStatus: true, timeStart: true, timeEnd: true, meetingType: true }
  });
  if (!booking) return NextResponse.json({ ok: false, error: "Booking not found" }, { status: 404 });
  ...
  return NextResponse.json({ ok: true, data: { eligible: capacityFull || urgent, capacityFull, urgent, environmentId: myEnvId } });
}
