...
const validateBookingData = (
  ...
  if (
    !data.ownerSurname ||
    typeof data.ownerSurname !== "string" ||
    data.ownerSurname.trim().length === 0
  ) {
  ...
  if (
    !data.ownerTel ||
    typeof data.ownerTel !== "string" ||
    data.ownerTel.trim().length === 0
  ) {
  ...
  if (
    ...
    typeof data.meetingDetail !== "string"
  ) {
  ...
  if (data.interpreterId !== undefined) {
    if (
      data.interpreterId !== null &&
      (!Number.isInteger(data.interpreterId) || data.interpreterId <= 0)
    ) {
  ...
  if (
    ...
    (!Array.isArray(data.inviteEmails) ||
      !data.inviteEmails.every(
        (email: string) => typeof email === "string" && isValidEmail(email)
      ))
  ) {
