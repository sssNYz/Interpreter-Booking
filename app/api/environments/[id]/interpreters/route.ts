...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Check target user exists and has INTERPRETER role
  ...
  if (!targetRoles.has("INTERPRETER")) {
    return NextResponse.json({ ok: false, error: "Employee is not INTERPRETER" }, { status: 400 });
  }

  // Rule: one interpreter can stay in only one environment
  const existing = await prisma.environmentInterpreter.findFirst({ where: { interpreterEmpCode: empCode } });
  if (existing && existing.environmentId !== envId) {
    return NextResponse.json({ ok: false, error: "Interpreter already in another environment" }, { status: 409 });
  }

  // Upsert for this env
  const link = await prisma.environmentInterpreter.upsert({
    where: { unique_env_interpreter: { environmentId: envId, interpreterEmpCode: empCode } },
    create: { environmentId: envId, interpreterEmpCode: empCode },
    update: {},
  });
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const link = await prisma.environmentInterpreter.findUnique({
    where: { unique_env_interpreter: { environmentId: envId, interpreterEmpCode: empCode } },
  });
  if (!link) return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });

  await prisma.environmentInterpreter.delete({ where: { id: link.id } });
