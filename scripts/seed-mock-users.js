...
async function seedMockUsers() {
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      console.log('ℹ️  Skipping existing users...');
    }
    ...
    if (newUsers.length === 0) {
      console.log('ℹ️  All users already exist in database');
      return;
    }
    ...
    for (let i = 0; i < newUsers.length; i += batchSize) {
      ...
      console.log(`   ✅ Created batch ${Math.floor(i/batchSize) + 1}: ${batch.map(u => u.code).join(', ')}`);
    }
