...
async function checkConsecutiveDRAssignmentHistory(
  ...
  const { applyDRPolicyRules, canOverrideDRPolicy } = await import('../config/policy');
...
export async function checkDRAssignmentWithOverride(
  ...
  const { applyDRPolicyRules } = await import('../config/policy');
  const overrideResult = applyDRPolicyRules(
    ...
    params.drPolicy,
    {
      ...
      interpreterListSize: 0 // Could be calculated from available interpreters
    }
  );
