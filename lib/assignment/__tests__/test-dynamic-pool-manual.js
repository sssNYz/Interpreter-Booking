  ...
  bookingPlan: {
    findMany: async (query) => {
      ...
      if (query.select && query.select.interpreterEmpCode && !query.select.id) {
        // For detectPoolSizeChanges - return distinct interpreters
...
async function runTests() {
  try {
    ...
    const fairnessAdjustments = await adjustFairnessForNewInterpreters(interpreterPool, 30);
