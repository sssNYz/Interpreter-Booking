...
type ApiResponse = {
  months: MonthName[];
  interpreters: string[];
  totalHoursLineMinutes: Array<{ month: MonthName; total: number } & Record<string, number>>;
  hoursFooter: { perInterpreter: number[]; grand: number; diff: number };
  year: number;
};
...
export function HoursTab({ year }: { year: number }) {
  ...
  const interpreterColors = React.useMemo<Record<string, string>>(() => {
    if (!data) return {};
    const palette = [
  ...
  return (
    <>
      <Card className="h-[380px] mb-4">
        <CardHeader className="pb-0"><CardTitle className="text-base">Total Time per Month (Year {data.year})</CardTitle></CardHeader>
        <CardContent className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={totalHoursLineMinutes}>
              ...
              <Legend />
              {interpreters.map((p) => (<Bar key={p} dataKey={p} fill={interpreterColors[p]} name={p} />))}
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Monthly Time Summary (displayed as hr/min) – Year {data.year} with Diff</CardTitle></CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-100 dark:bg-slate-800">
                  ...
                  {interpreters.map((p) => (<th key={p} className="p-2 text-right">{p}</th>))}
              ...
              <tbody>
                {totalHoursLineMinutes.map((r) => {
                  const vals = interpreters.map((p) => (r[p] as number) || 0);
                  ...
                  return (
                    <tr key={r.month} className="border-b odd:bg-white even:bg-muted/30 hover:bg-muted/40">
                      ...
                      {interpreters.map((p) => (<td key={p} className="p-2 text-right">{formatHoursDecimal((r[p] as number) || 0)}</td>))}
                ...
                <tr className="bg-emerald-50 text-emerald-900 font-semibold">
                  ...
                  {hoursFooter.perInterpreter.map((v, idx) => (<td key={idx} className="p-2 text-right">{formatHoursDecimal(v)}</td>))}
